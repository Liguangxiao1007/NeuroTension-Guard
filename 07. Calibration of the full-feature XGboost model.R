...
data_train <- read.xlsx('./data/train_pred_all.xlsx', sheet = 1)
observed_dif <- data_train$Dementia_dif
predicted_dif <- data_train$pre_dementia_dif
...
cat("Training set ATE Expected Calibration Error (ECE):", round(ate_ece_result$ece, 5), "\n")

# Print detailed bin information
cat("Training set ATE ECE bin details:\n")
...
cat("\n=== Training Set ATE Prediction Calibration Metrics Summary ===\n")
